...
// --- CONFIGURATION (Aapka Official Firebase Config) ---
...
// MANDATORY PATH STRUCTURE (Rule 1)
const COLLECTION_PATH = ['artifacts', appId, 'public', 'data', 'players'];

// Helper: Image Compression (Taki database fast chale aur crash na ho)
...
export default function App() {
  ...
  // --- 1. AUTHENTICATION (Anonymous Login for Security) ---
  useEffect(() => {
  ...
  // --- 2. DATA SYNC (Real-time update from Firestore) ---
  useEffect(() => {
    if (!user) return;
    const playersRef = collection(db, ...COLLECTION_PATH);
    const unsubscribe = onSnapshot(playersRef, (snapshot) => {
      const list = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // Sort by latest timestamp (Manual sort to avoid composite index error)
  ...
  // --- 3. EXCEL DOWNLOAD (CSV Export Feature) ---
  const exportToExcel = () => {
    if (players.length === 0) return alert("Database mein koi data nahi mila!");
    
    const headers = ["ID", "Name", "Category", "Age", "Contact", "Native Place", "Payment Status", "Auction Status", "Team"];
    const rows = players.map(p => [
      p.id, p.name, p.category, p.age, p.contact, p.native, p.paymentStatus, p.auctionStatus, p.team || "-"
    ]);
    ...
    link.setAttribute("download", `NPCC_Players_Report_${new Date().toLocaleDateString()}.csv`);
    ...
    <nav className="bg-[#5c3a21] text-white p-4 shadow-xl sticky top-0 z-50">
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <div className="flex gap-4 text-xs font-bold uppercase tracking-widest">
          ...
          {!isAdmin ? (
            <button onClick={() => navigate('admin-login')} className="text-orange-200 border border-white/20 px-2 rounded">Admin Portal</button>
          ) : (
            <button onClick={() => {setIsAdmin(false); navigate('landing');}} className="text-red-300">Logout Admin</button>
          )}
        </div>
      </div>
    </nav>
  ...
  const Landing = () => (
    <div className="max-w-4xl mx-auto mt-12 px-4 text-center pb-20">
      ...
      <p className="text-gray-500 mb-12 font-bold uppercase text-[10px] tracking-[4px]">Category Select Karein Aur Register Karein</p>
      ...
      <div className="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
        <button onClick={() => { setCategory('Youth'); navigate('register'); }} className="bg-white p-8 rounded-[40px] shadow-2xl border-2 border-transparent hover:border-blue-500 transition-all text-left group">
          ...
          <p className="text-gray-400 text-[10px] mb-6 font-black uppercase">15 to 35 Saal Tak</p>
          <div className="text-blue-600 font-black flex items-center gap-1 uppercase text-sm">Abhi Register Karein <ArrowRight size={18}/></div>
        </button>
        <button onClick={() => { setCategory('40+'); navigate('register'); }} className="bg-white p-8 rounded-[40px] shadow-2xl border-2 border-transparent hover:border-orange-500 transition-all text-left group">
          ...
          <p className="text-gray-400 text-[10px] mb-6 font-black uppercase tracking-widest">40 Saal aur Upar</p>
          <div className="text-orange-700 font-black flex items-center gap-1 uppercase text-sm">Abhi Register Karein <ArrowRight size={18}/></div>
        </button>
      </div>
      ...
      >
        View Registration & Auction Pool
      </button>
    </div>
  );
  ...
  const Register = () => {
    ...
    const handleNext = (e) => {
      e.preventDefault();
      // DUPLICATE CHECK: Block number if already in database
      const isAlreadyRegistered = players.some(plr => plr.contact === form.contact);
      if(isAlreadyRegistered) return alert("❌ Ye mobile number pehle se registered hai! Directory mein check karein.");

      if(!photo) return alert("Kheladi ki photo upload karein!");
      setTempPlayer({ 
        ...form, 
        category, 
        photoUrl: photo, 
    ...
    return (
      <form onSubmit={handleNext} className="max-w-xl mx-auto mt-8 p-10 bg-white rounded-[45px] shadow-2xl space-y-6 mx-4 border-b-[10px] border-[#5c3a21]">
        <h2 className="text-3xl font-black text-center text-[#5c3a21] uppercase italic tracking-tighter underline underline-offset-8 decoration-orange-400">Player Form</h2>
        <div className="space-y-4">
          <input required placeholder="KHELADI KA POORA NAAM" className="w-full p-4 bg-gray-50 border-2 rounded-2xl font-bold uppercase focus:border-[#5c3a21] outline-none shadow-inner" onChange={e => setForm({...form, name: e.target.value})} />
          <div className="grid grid-cols-2 gap-4">
            <input required type="number" placeholder="AGE (UMAR)" className="w-full p-4 bg-gray-50 border-2 rounded-2xl font-bold focus:border-[#5c3a21] outline-none shadow-inner" onChange={e => setForm({...form, age: e.target.value})} />
            <input required placeholder="MOBILE NUMBER" className="w-full p-4 bg-gray-50 border-2 rounded-2xl font-bold focus:border-[#5c3a21] outline-none shadow-inner" onChange={e => setForm({...form, contact: e.target.value})} />
          </div>
          <input required placeholder="NATIVE PLACE (CITY/TOWN)" className="w-full p-4 bg-gray-50 border-2 rounded-2xl font-bold uppercase focus:border-[#5c3a21] outline-none shadow-inner" onChange={e => setForm({...form, native: e.target.value})} />
        </div>
  ...
  const Payment = () => {
    ...
    // QR generator using dynamic link
    ...
    const handleFinalSubmit = async () => {
      ...
      try {
        ...
        alert("DATA NOT SAVED! Error: " + err.message); 
      }
      setSaving(false);
    };
    ...
    return (
      <div className="max-w-md mx-auto mt-8 p-8 bg-white rounded-[50px] shadow-2xl text-center border-t-[10px] border-green-500 mx-4">
        <h2 className="text-3xl font-black text-gray-800 mb-6 italic tracking-tighter uppercase underline decoration-green-300">Scan & Pay ₹500</h2>
        <div className="bg-blue-50 p-6 rounded-[35px] mb-8 shadow-inner border-2 border-blue-100">
          ...
          <p className="text-[11px] font-black text-blue-600 bg-blue-100 py-1 px-3 rounded-full inline-block mt-2 tracking-widest uppercase italic">{upiId}</p>
        </div>
        <div className="border-4 border-dotted p-6 relative bg-gray-50 rounded-3xl cursor-pointer mb-8 border-gray-200">
          {ss ? (
            ...
            <div className="text-gray-300 font-black uppercase text-[10px] tracking-widest flex flex-col items-center gap-2">
              <CreditCard size={32}/> UPLOAD G-PAY SCREENSHOT
            </div>
          )}
  ...
  const Success = () => (
    <div className="max-w-md mx-auto mt-20 text-center p-12 bg-white rounded-[60px] shadow-2xl mx-4 border-b-[10px] border-green-500">
      ...
      <h2 className="text-4xl font-black text-gray-800 tracking-tighter italic uppercase">Submission Done!</h2>
      <p className="text-gray-400 mt-4 font-bold text-sm uppercase tracking-widest italic tracking-tighter leading-relaxed">Aapka data safe ho gaya hai.<br/>Admin verification ka intezar karein.</p>
      <button onClick={() => navigate('directory')} className="mt-12 w-full bg-[#5c3a21] text-white py-5 rounded-[30px] font-black uppercase italic shadow-2xl tracking-tighter">View Pool Status</button>
    </div>
  );
    ...
    return (
      <div className="max-w-6xl mx-auto mt-8 p-6 pb-20">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
           <h2 className="text-4xl font-black text-[#5c3a21] uppercase italic tracking-tighter underline decoration-orange-400">Players Pool</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
          ...
          {list.length === 0 && <div className="col-span-full py-20 text-center font-black text-gray-300 animate-pulse uppercase tracking-[10px]">Wait: Loading Cloud Data...</div>}
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="max-w-7xl mx-auto mt-10 p-4 pb-40">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6 px-4">
           <h2 className="text-4xl font-black text-[#5c3a21] uppercase italic tracking-tighter decoration-orange-400 underline underline-offset-8">Admin Console</h2>
           <button onClick={exportToExcel} className="flex items-center gap-3 bg-green-600 text-white px-10 py-4 rounded-full font-black text-sm shadow-2xl hover:bg-green-700 transition active:scale-95">
             <FileSpreadsheet size={22}/> DOWNLOAD DATA SHEET (CSV)
           </button>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[800px]">
              <thead className="bg-gray-100 border-b-2">
                <tr><th className="p-8 font-black text-xs uppercase tracking-widest text-gray-500">Player Profile</th><th className="p-8 font-black text-xs uppercase tracking-widest text-gray-500 text-center">Payment</th><th className="p-8 font-black text-xs uppercase tracking-widest text-gray-500 text-center">Auction Status & Team</th></tr>
              </thead>
              <tbody>
                {players.map(p => (
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-orange-50/20 transition-all">
                    <td className="p-8 flex items-center gap-5">
                      ...
                      <div><div className="font-black text-[#5c3a21] uppercase italic text-lg tracking-tighter">{p.name}</div><div className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{p.category} • {p.contact}</div></div>
                    </td>
                    ...
                    <td className="p-8 text-center">
                      <div className="flex items-center gap-2 justify-center">
                        {/* SOLD / UNSOLD DROPDOWN */}
                        <select className="text-[11px] font-black border-2 rounded-xl p-2 uppercase outline-none focus:border-[#5c3a21] bg-gray-50" value={p.auctionStatus} onChange={(e) => upd(p.id, { auctionStatus: e.target.value })}>
                          <option value="Unsold">UNSOLD</option><option value="Sold">SOLD</option>
                        </select>
                        {/* TEAM NAME INPUT */}
                        <input placeholder="Enter Team" className="text-[11px] font-black border-2 p-2 rounded-xl w-32 uppercase focus:border-[#5c3a21] outline-none shadow-inner" value={p.team === '-' ? '' : p.team} onChange={(e) => upd(p.id, { team: e.target.value })} />
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        {sel && (
          <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-[999] backdrop-blur-xl">
            <div className="bg-white p-10 rounded-[60px] w-full max-w-lg text-center relative shadow-2xl border-8 border-orange-50 animate-in zoom-in duration-200">
              <button onClick={() => setSel(null)} className="absolute -top-6 -right-6 bg-[#5c3a21] text-white rounded-full p-4 shadow-2xl border-4 border-white active:scale-90 transition-all"><X size={28}/></button>
              <h3 className="font-black text-2xl mb-8 text-[#5c3a21] italic uppercase underline decoration-orange-400 tracking-tighter italic">Proof Review</h3>
              <div className="bg-gray-100 p-3 rounded-[40px] mb-10 border-4 border-gray-50 shadow-inner overflow-hidden">
                <img src={sel.screenshot} className="max-h-[450px] mx-auto rounded-[30px] shadow-2xl object-contain" />
              </div>
              <div className="flex gap-4">
                <button onClick={() => setSel(null)} className="flex-1 py-5 bg-gray-100 rounded-[30px] font-black text-gray-400 italic text-sm">NOT PAID</button>
                <button onClick={async () => { await upd(sel.id, { paymentStatus: 'Paid' }); setSel(null); }} className="flex-1 py-5 bg-green-600 text-white rounded-[30px] font-black shadow-2xl italic text-sm tracking-tighter uppercase active:scale-95 transition-all">APPROVE PLAYER</button>
              </div>
            </div>
          </div>
        )}
  ...
  const AdminLogin = () => {
    ...
    return (
      <div className="max-w-md mx-auto mt-20 p-12 bg-white shadow-2xl rounded-[50px] border-t-[10px] border-[#5c3a21] text-center mx-4">
        ...
        <p className="text-[10px] font-bold text-gray-300 tracking-[3px] uppercase italic">Use secret key to access control panel</p>
      </div>
    );
  };

  if (loading) return <div className="h-screen flex flex-col items-center justify-center bg-[#f8f5f0] text-[#5c3a21] font-black tracking-widest uppercase text-xs italic animate-pulse"><RefreshCcw className="animate-spin mb-6" size={60}/><p>Accessing NPCC Cloud...</p></div>;
