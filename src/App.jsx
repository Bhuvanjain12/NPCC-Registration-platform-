...
const Landing = ({ navigate, setCategory, regControl }) => (
  <div className="max-w-6xl mx-auto mt-16 px-4 text-center pb-24 relative">
    ...
    <p className="text-slate-500 mb-16 font-bold text-lg max-w-2xl mx-auto uppercase tracking-widest italic opacity-80 leading-relaxed">
      Celebrating Cricket with Colors<br/>Join the Ultimate Spring Championship
    </p>
    <div className="grid md:grid-cols-2 gap-10 max-w-4xl mx-auto">
      ...
      >
        ...
        <Zap className={`${!regControl.youthOpen ? 'text-gray-400' : 'text-pink-600'} mb-8 relative z-10`} size={56} />
        <h2 className="text-4xl font-black mb-2 relative z-10 text-slate-800 uppercase italic leading-none">Youth Pool</h2>
        <p className="text-pink-400 text-xs mb-8 relative z-10 font-black tracking-widest uppercase">{regControl.youthOpen ? 'Registration Live' : 'Closed'}</p>
        <div className={`${!regControl.youthOpen ? 'bg-gray-400' : 'bg-pink-600'} text-white w-max px-8 py-3 rounded-full font-black flex items-center gap-2 uppercase text-xs relative z-10 shadow-lg`}>
          {regControl.youthOpen ? 'Join Now' : 'Closed'} <ArrowRight size={16}/>
        </div>
      </button>
      ...
      >
        ...
        <Trophy className={`${!regControl.fortyPlusOpen ? 'text-gray-400' : 'text-purple-600'} mb-8 relative z-10`} size={56} />
        <h2 className="text-4xl font-black mb-2 relative z-10 text-slate-800 uppercase italic leading-none">40+ League</h2>
        <p className="text-purple-400 text-xs mb-8 relative z-10 font-black tracking-widest uppercase">{regControl.fortyPlusOpen ? 'Veterans Live' : 'Closed'}</p>
        <div className={`${!regControl.fortyPlusOpen ? 'bg-gray-400' : 'bg-purple-600'} text-white w-max px-8 py-3 rounded-full font-black flex items-center gap-2 uppercase text-xs relative z-10 shadow-lg`}>
          {regControl.fortyPlusOpen ? 'Join Now' : 'Closed'} <ArrowRight size={16}/>
        </div>
      </button>
    </div>
    <div className="mt-24">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-1">
                {p.availability && Object.entries(p.availability).filter(([_, v]) => v).map(([date]) => (
                  <span key={date} className="bg-white/20 backdrop-blur-md text-white text-[7px] font-black px-2 py-1 rounded-full uppercase tracking-tighter border border-white/30">
                    Avail: {date.replace('apr', ' Apr')}
                  </span>
                ))}
              </div>
            </div>
            <div className="p-10 text-center bg-white">
              <h3 className="font-black text-3xl text-slate-800 uppercase leading-none mb-3 tracking-tighter italic">{p.name}</h3>
    ...
    <div className="max-w-7xl mx-auto mt-12 p-6 pb-40">
      ...
      {sel && (
        <div className="fixed inset-0 bg-slate-900/95 flex items-center justify-center p-4 z-[999] backdrop-blur-2xl">
          <div className="bg-white p-16 rounded-[5rem] w-full max-w-2xl text-center relative shadow-2xl border border-white/20">
            ...
            <h3 className="font-black text-4xl mb-10 text-slate-800 italic uppercase underline decoration-pink-500">Transaction Proof</h3>
...
export default function App() {
  ...
  const availDates = category === 'Youth' ? ['4apr', '5apr'] : ['3apr', '4apr', '5apr'];
      ...
      <main>
        ...
        {view === 'register' && (
          <form onSubmit={async (e) => {
            ...
            if (Object.values(availability).filter(v => v).length === 0) return alert("Please select your availability dates!");
            ...
            <div className="space-y-6">
              <input name="name" required placeholder="FULL PLAYER NAME" className="w-full p-6 bg-slate-50 rounded-[2rem] font-black uppercase focus:border-pink-500 border-2 border-transparent outline-none transition-all shadow-inner" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                ...
                <input name="dob" required type="date" className="w-full p-6 bg-slate-50 rounded-[2rem] font-black focus:border-pink-500 border-2 border-transparent outline-none transition-all shadow-inner" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <input name="contact" required placeholder="WHATSAPP CONTACT" className="w-full p-6 bg-slate-50 rounded-[2rem] font-black focus:border-pink-500 border-2 border-transparent outline-none transition-all shadow-inner" />
                <input name="native" required placeholder="HOME TOWN" className="w-full p-6 bg-slate-50 rounded-[2rem] font-black uppercase focus:border-pink-500 border-2 border-transparent outline-none transition-all shadow-inner" />
              </div>
            </div>

            {/* AVAILABILITY TICK BOXES */}
            <div className="bg-pink-50/50 p-8 rounded-[2.5rem] border border-pink-100">
               <p className="font-black text-slate-800 uppercase text-xs tracking-widest mb-6 flex items-center gap-2">
                 <Calendar size={16} className="text-pink-500" /> Tournament Availability
               </p>
               <div className="flex flex-wrap gap-4">
                 {availDates.map(date => (
                   <label key={date} className={`flex-1 min-w-[100px] flex items-center justify-center gap-3 p-4 rounded-2xl cursor-pointer transition-all border-2 ${availability[date] ? 'bg-pink-600 border-pink-600 text-white shadow-lg shadow-pink-200' : 'bg-white border-pink-100 text-pink-300'}`}>
                     ...
                     <span className="font-black uppercase text-[10px] tracking-widest">{date.replace('apr', ' Apr')}</span>
            ...
            <div className="border-4 border-dashed p-10 text-center rounded-[3rem] bg-slate-50 border-slate-200 relative group">
              ...
              <span className="text-[10px] font-black text-pink-300 uppercase tracking-widest">Select Player Photo</span>
        ...
        {view === 'payment' && (
          <div className="max-w-xl mx-auto mt-12 p-12 bg-white rounded-[4rem] shadow-2xl text-center border-t-[15px] border-pink-500 mx-4">
             <h2 className="text-4xl font-black text-slate-800 mb-8 italic tracking-tighter uppercase leading-none underline decoration-pink-200">Confirmation</h2>
             <div className="bg-gradient-to-b from-pink-50 to-white p-12 rounded-[3.5rem] mb-10 border border-pink-100 shadow-inner">
               <img src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(`upi://pay?pa=bjain6851@okaxis&pn=Bhuvan%20Jain&cu=INR`)}`} className="w-64 mx-auto mb-8 rounded-3xl shadow-2xl bg-white p-4" />
               <p className="font-black text-slate-800 text-2xl uppercase mb-1 leading-none tracking-tighter">Bhuvan Jain</p>
               ...
               <p className="text-xs font-bold text-slate-400 uppercase tracking-widest italic">Scan to confirm championship spot</p>
             </div>
             ...
             <button onClick={async () => {
               if(!tempPlayer.screenshot) return alert("Please upload payment screenshot!");
               try {
        ...
        {view === 'admin-login' && (
          <div className="max-w-md mx-auto mt-24 p-16 bg-white rounded-[5rem] shadow-2xl text-center border border-pink-50 mx-4">
            ...
            <h2 className="text-4xl font-black mb-12 uppercase italic text-slate-800 tracking-tighter">Admin Access</h2>
            <input type="password" placeholder="SECRET KEY" className="w-full p-8 bg-slate-50 border-2 rounded-[2.5rem] text-center font-black text-2xl tracking-[0.4em] outline-none focus:border-pink-500 transition-all shadow-inner" onChange={e => {if(e.target.value === 'bababhuvandev') {setIsAdmin(true); navigate('admin')}}} />
          </div>
        )}
        ...
        {view === 'success' && (
          <div className="max-w-xl mx-auto mt-24 text-center p-20 bg-white rounded-[6rem] shadow-2xl mx-4 border-b-8 border-emerald-500">
            ...
            <button onClick={() => navigate('directory')} className="w-full bg-slate-900 text-white py-8 rounded-full font-black uppercase italic tracking-widest text-xs shadow-2xl hover:-translate-y-1 transition-all">Check Live Pool Directory</button>
          </div>
        )}
      </main>
    </div>
