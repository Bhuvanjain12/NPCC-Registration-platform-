...
import { 
  ...
  Activity, Info, Sparkles, Palette, ToggleLeft, ToggleRight, Loader2, DownloadCloud
} from 'lucide-react';
...
// Aggressive Compression for Speed & Mobile Stability
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const MAX_SIZE = isScreenshot ? 450 : 180; 
        let width = img.width;
...
const Navbar = memo(({ navigate, isAdmin, setIsAdmin }) => (
  <nav className="bg-white/95 backdrop-blur-xl p-4 shadow-xl sticky top-0 z-50 border-b border-pink-100 font-sans">
...
const Landing = memo(({ navigate, setCategory, regControl }) => (
  <div className="max-w-6xl mx-auto mt-12 px-4 text-center pb-24 relative font-sans">
    ...
    <p className="text-slate-500 mb-16 font-bold text-lg max-w-2xl mx-auto uppercase tracking-widest italic opacity-80 leading-relaxed">
      Celebrating the Spirit of Colors & Cricket
    </p>
...
const PlayerDirectory = ({ players, filterType, setFilterType, searchTerm, setSearchTerm }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto mt-12 p-6 pb-32 font-sans">
      ...
      {filteredList.length === 0 && <div className="text-center py-20 font-black text-slate-200 text-4xl uppercase tracking-widest italic italic">Pool is empty</div>}
    </div>
  );
};
...
const AdminDashboard = ({ players, exportData, upd, regControl, updateControl }) => {
  ...
  const downloadPhoto = (base64, name) => {
    const link = document.createElement('a');
    link.href = base64;
    link.download = `${name.replace(/\s+/g, '_')}_NPCC_Photo.jpg`;
  ...
  return (
    <div className="max-w-7xl mx-auto mt-12 p-6 pb-40 font-sans">
      ...
      <div className="bg-white p-10 rounded-[3rem] shadow-2xl mb-16 border border-pink-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
           <div className="bg-slate-50 p-6 rounded-[2rem] flex items-center justify-between border-2 border-transparent hover:border-pink-200 transition-all">
             ...
             <p className={`text-[10px] font-bold uppercase tracking-widest mt-2 ${regControl.youthOpen ? 'text-emerald-500' : 'text-red-500'}`}>Currently {regControl.youthOpen ? 'Open' : 'Closed'}</p></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[1200px]">
            <thead className="bg-slate-50 border-b-2 font-black text-[10px] uppercase tracking-[0.2em] text-slate-400">
              <tr>
                <th className="p-12">Candidate</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sans">
              {players.map(p => (
                <tr key={p.id} className="hover:bg-pink-50/20 transition-all group">
                  ...
                  <td className="p-12 text-center">
                    <button 
                      onClick={() => downloadPhoto(p.photoUrl, p.name)}
                      className="bg-white border-2 border-pink-100 text-pink-500 p-4 rounded-2xl shadow-sm hover:bg-pink-500 hover:text-white hover:shadow-pink-200 transition-all active:scale-90 flex flex-col items-center gap-1 mx-auto group"
                    >
                  ...
                  <td className="p-12">
                    <div className="flex items-center gap-6 justify-center">
                      <select className="bg-slate-50 border-2 border-transparent rounded-[1.5rem] p-4 font-black text-[10px] uppercase outline-none focus:border-pink-500 transition-all" value={p.auctionStatus} onChange={(e) => upd(p.id, { auctionStatus: e.target.value })}>
                        <option value="Unsold">In Pool</option><option value="Sold">Sold</option>
                      </select>
                      <input placeholder="TEAM NAME" className="bg-slate-50 border-2 border-transparent rounded-[1.5rem] p-4 w-56 font-black text-[10px] uppercase outline-none focus:bg-white focus:border-pink-500 shadow-inner transition-all" value={p.team === '-' ? '' : p.team} onChange={(e) => upd(p.id, { team: e.target.value })} />
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {sel && (
        <div className="fixed inset-0 bg-slate-900/95 flex items-center justify-center p-4 z-[999] backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white p-16 rounded-[5rem] w-full max-w-2xl text-center relative shadow-2xl border border-white/10">
            ...
            <div className="flex gap-4">
              <button onClick={() => downloadPhoto(sel.screenshot, `${sel.name}_Screenshot`)} className="flex-1 py-7 bg-slate-100 text-slate-600 rounded-full font-black uppercase tracking-widest text-xs hover:bg-slate-200 transition-all">Download Proof</button>
              <button onClick={async () => { await upd(sel.id, { paymentStatus: 'Paid' }); setSel(null); }} className="flex-[2] py-7 bg-emerald-600 text-white rounded-full font-black shadow-2xl uppercase tracking-widest text-xs hover:bg-emerald-700 transition-all active:scale-95">Verify & Approve</button>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#fdfaf8] text-slate-900 font-black tracking-[0.5em] uppercase text-xs italic">
      ...
      <p className="animate-pulse">Connecting Championship Server...</p>
    </div>
  );
      ...
      <main className="animate-in fade-in duration-500">
        ...
        {view === 'register' && (
          <form onSubmit={async (e) => {
            ...
            <div className="border-4 border-dashed p-10 text-center rounded-[3rem] bg-slate-50 border-slate-200 relative group flex flex-col items-center justify-center min-h-[160px] active:scale-95 transition-all">
        ...
        {view === 'payment' && (
          <div className="max-w-xl mx-auto mt-12 p-12 bg-white rounded-[4rem] shadow-2xl text-center border-t-[15px] border-pink-500 mx-4 font-sans relative overflow-hidden">
             ...
             <div className="border-4 border-dotted p-12 bg-slate-50 rounded-[3rem] border-slate-200 relative mb-10 group hover:border-pink-400 min-h-[160px] flex flex-col items-center justify-center z-10 active:scale-95 transition-all">
             ...
             <button onClick={async () => {
               ...
               try { await setDoc(doc(db, ...COLLECTION_PATH, tempPlayer.id), tempPlayer); navigate('success'); } catch(err) { alert("Submission failed. Check internet."); }
        ...
        {view === 'admin-login' && (
          <div className="max-w-md mx-auto mt-24 p-16 bg-white rounded-[5rem] shadow-2xl text-center border border-pink-50 mx-4">
            <Shield size={48} className="mx-auto text-pink-500 mb-10 shadow-lg p-2 bg-pink-50 rounded-2xl" />
            <h2 className="text-4xl font-black mb-12 uppercase italic text-slate-800 tracking-tighter leading-none font-sans">Admin Portal</h2>
        ...
        {view === 'success' && (
          <div className="max-w-xl mx-auto mt-24 text-center p-20 bg-white rounded-[6rem] shadow-2xl mx-4 border-b-8 border-emerald-500 font-sans">
            ...
            <h2 className="text-6xl font-black text-slate-900 italic uppercase tracking-tighter leading-[0.8] mb-8 underline decoration-emerald-200">Success!</h2>
            <button onClick={() => navigate('directory')} className="w-full bg-slate-900 text-white py-8 rounded-full font-black uppercase italic tracking-widest text-xs shadow-2xl active:scale-95 hover:-translate-y-1 transition-all">View Live Directory</button>
          </div>
        )}
      </main>
    </div>
